  ...
  const user = [
    ...
    '- SR：attr 里的百分比类字段（cpct/cdmg/stance/effPct/effDef/recharge 等）已经是“百分比数值”，不要再 *100。',
	    ...
	    '- buffs: 「无视X%防御」=> ignore / aIgnore/eIgnore/qIgnore/tIgnore；「防御力降低X%」=> enemyDef（不要把无视防御写成 enemyDef）。',
  	    ...
  	    '  - 叠层线性倍率（常见于 SR/强化机制）：若描述出现「每层…提高…(伤害)倍率/倍率提高」且可用表名中存在「每层倍率」类表（例如 主目标每层倍率/相邻目标每层倍率/每层伤害提高），请在 details 中至少输出 1 条按最大层数计算的“满层/最大层数”展示行（title 写明层数），并在 dmgExpr 中把“每层倍率 * 层数(必要时再乘常量)”线性叠加到 base 倍率后再 dmg(...)。',
  	    '    - 示例：dmg(talent.e2["技能伤害"] + (talent.t["主目标每层倍率"] || 0) * params.stacks, "e")；多目标同理分别用主/相邻表并在 (完整) 行做合计。',
