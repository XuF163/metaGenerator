...
import { buildCalcJsWithLlmOrHeuristic, CALC_CREATED_BY } from '../generate/calc/llm-calc.js'
...
function isOutdatedAutoGeneratedCalcRaw(raw: string): boolean {
  if (!isAutoGeneratedCalcRaw(raw)) return false
  // Bump `CALC_CREATED_BY` when generator semantics change so `calc` can upgrade deterministically without `--force`.
  return !raw.includes(`export const createdBy = ${JSON.stringify(CALC_CREATED_BY)}`)
}
...
export async function calcCommand(ctx: CommandContext, options: GenOptions): Promise<void> {
  ...
  for (const game of games) {
    ...
    for await (const filePath of walkFiles(charRoot, { ignoreNames: new Set(['.ace-tool']) })) {
      ...
      const isOutdated = isAutoGenerated && isOutdatedAutoGeneratedCalcRaw(calcRaw)
      const shouldUpgrade =
        isPlaceholder ||
        isInvalid ||
        isOutdated ||
