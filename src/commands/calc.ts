 ...
 * - If it is a placeholder (or when `--force` is set), generate a minimal usable calc.js:
 *   - Prefer LLM (OpenAI-compatible) when configured
 *   - Fall back to heuristic when LLM is unavailable
 *
 * NOTE (security / trust boundary):
 * - This command must NOT copy any files from baseline meta into the output. Baseline meta is for validate/compare only.
 */
