 ...
 *
 * Extra (upstream-direct / baseline parity):
 * - When `calc.channel="upstream-direct"` and a baseline meta root is provided, we treat the baseline `calc.js` as the
 *   upstream source of truth and sync it into the generated output (only overwriting placeholder/auto-generated files).
 *   This is the most reliable way to keep runtime damage results consistent with the baseline meta.
 */
...
export async function calcCommand(ctx: CommandContext, options: GenOptions): Promise<void> {
  ...
  ctx.log.info(`[meta-gen] calc: baselineRoot=${baselineRoot}`)
  ...
  const canSyncBaseline = calcChannel === 'upstream-direct'

  // Sync baseline calc dependencies (artifact/weapon JS tables) to keep runtime damage consistent.
  // Note: this is intentionally coarse-grained; in upstream-direct mode we expect baseline parity to be the goal.
  if (canSyncBaseline) {
    let synced = 0
    for (const game of games) {
      const baselineMetaRoot = metaGameDir(baselineRoot, game)
      const outputMetaRoot = metaGameDir(outputRoot, game)
      for (const relDir of ['artifact', 'weapon']) {
        const srcRoot = path.join(baselineMetaRoot, relDir)
        if (!fs.existsSync(srcRoot)) continue
        for await (const srcPath of walkFiles(srcRoot, { ignoreNames: new Set(['.ace-tool']) })) {
          if (path.extname(srcPath) !== '.js') continue
          const rel = path.relative(srcRoot, srcPath)
          if (!rel || rel.startsWith('..')) continue
          const dstPath = path.join(outputMetaRoot, relDir, rel)
    ...
    if (synced) ctx.log.info(`[meta-gen] calc: baseline js synced (artifact/weapon): ${synced}`)
  }
  ...
  for (const game of games) {
    ...
    for await (const filePath of walkFiles(charRoot, { ignoreNames: new Set(['.ace-tool']) })) {
      ...
      const shouldSyncBaseline =
        canSyncBaseline &&
        (isPlaceholder || isAutoGenerated) &&
        fs.existsSync(baselineMetaRoot) &&
        (() => {
          const rel = path.relative(outputMetaRoot, filePath)
          if (!rel || rel.startsWith('..')) return false
          const baselineCalcPath = path.join(baselineMetaRoot, rel)
          return fs.existsSync(baselineCalcPath)
        })()
    ...
    if (canSyncBaseline) {
      try {
        ...
        if (rel && !rel.startsWith('..')) {
          const baselineCalcPath = path.join(baselineMetaRoot, rel)
          if (fs.existsSync(baselineCalcPath)) {
            const baselineRaw = fs.readFileSync(baselineCalcPath, 'utf8')
            // Avoid syncing placeholders from baseline (fallback to generator instead).
            // NOTE: baseline calc.js may contain ESM `import` statements (e.g. `import { Format } from "#miao"`),
            // which our lightweight `new Function()` validator cannot parse. Trust baseline here; runtime parity is
            // verified by circaltest panel-regression.
            if (baselineRaw && !baselineRaw.includes('Auto-generated placeholder')) {
              fs.writeFileSync(filePath, baselineRaw, 'utf8')
              upgraded++
              if (upgraded === 1 || upgraded % 20 === 0) {
                ctx.log.info(`[meta-gen] calc synced from baseline: ${upgraded}/${jobs.length} (scanned=${scanned})`)
              }
              return
            }
          }
        ...
        // best-effort; fall back to generation below
      }
    }
